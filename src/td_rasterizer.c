...
td_u8* ptexture_resize(const td_u8 *old, const td_u8 channel, const term_ivec2 old_size, const term_ivec2 new_size)
{
    float
...
TD_INLINE td_f32 edge_function(term_ivec2 v0, term_ivec2 v1, term_ivec2 v2) {
    ...
    const td_u8* texture_data)
{
    ...
    td_f32 inv_area = 1.0f / (float)(B2 * A1 - (pv2.pos.y - pv1.pos.y) * (pv3.pos.x - pv1.pos.x));
    // /if(area == 0) return;
